    ...
    <>
      {/* <div className="">
        ...
        <div>
          ...
          <p>Calificación: {movie.certification? "+"+movie.certification : 'Consulta en taquilla'} </p>
        </div>
      </div> */}
      ...
      <Link className="relative dark:bg-red-950  w-full rounded-3xl min-h-[340px] md:col-span-2 p-1 group overflow-hidden" to={`/movies/${movie.id}`}>
          <figure className="w-full rounded-3xl h-2/3 overflow-hidden">
            <img className="sm:min-h-[400px] w-full h-full  md:min-h-[500px] lg:min-h-[600px] object-cover group-hover:scale-105 group-hover:brightness-110 transition-all duration-300" src={`https://image.tmdb.org/t/p/original/${movie.poster_path}`} alt="movie poster" loading="lazy" />
          </figure>
          ...
          <div className="p-6">
            ...
            <ul className="flex gap-1 flex-wrap text-sm mt-4">
              {movie.genre_ids.map((genre) => (
                <li className="rounded-full px-2 border-dark-200 dark:border-dark-800 bg-dark-300/60 dark:bg-primary-900/30 text-dark-700 dark:text-dark-200">{genre}</li>
              ))}
            </ul>
          </div>

          <span className="absolute bottom-[33%] left-4 backdrop-blur-md bg-teal-600/50 py-1 px-2 rounded-full z-10 text-sm font-semibold text-white ">
